...
const ClientModal = ({ client, revenue, events, onClose, onSave }: { client: Client, revenue: number, events: CalendarEvent[], onClose: any, onSave: any }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-slate-900/90 border border-slate-700/50 rounded-3xl w-full max-w-4xl max-h-[90vh] shadow-2xl relative overflow-hidden flex flex-col backdrop-blur-xl" onClick={(e) => e.stopPropagation()}>
                <div className={`h-32 bg-gradient-to-r ${colorClass} p-8 flex items-end relative opacity-90`}>
                    <button onClick={onClose} className="absolute top-6 right-6 text-white/70 hover:text-white bg-black/20 p-2 rounded-full backdrop-blur-sm transition-colors"><X /></button>
                    <div className="flex items-center gap-6 translate-y-8">
                        ...
                        <div className="mb-2 w-full max-w-md">
                             <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Nome do Cliente" className="w-full text-4xl font-bold text-white bg-transparent outline-none drop-shadow-md placeholder-white/50 border-b border-transparent focus:border-white/50 transition-colors" autoFocus={isNew} />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-1 space-y-6">
                             <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/30">
                                ...
                                <input type="text" value={formData.contact} onChange={e => setFormData({...formData, contact: e.target.value})} placeholder="Email ou Telefone" className="w-full bg-transparent border-b border-slate-700 focus:border-blue-500 outline-none pb-1 text-white placeholder-slate-600" />
                             </div>
                             {!isNew && (
                                <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/30">
                             ...
                             <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/30 h-full">
                                ...
                                <textarea value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} placeholder="Preferências, anotações..." className="w-full h-32 bg-transparent outline-none text-slate-300 resize-none text-sm leading-relaxed placeholder-slate-600" />
                             </div>
                        </div>
                        <div className="lg:col-span-2">
                             ...
                             <div className="space-y-3">
                                ...
                                {events.length === 0 && <div className="text-center py-10 text-slate-500 italic">{isNew ? 'Salve o cliente para adicionar serviços.' : 'Nenhum serviço contratado ainda.'}</div>}
                             </div>
                        </div>
                    </div>
                </div>
                <div className="p-6 border-t border-slate-700/30 flex justify-end items-center bg-slate-900/30">
                    <button onClick={(e) => { e.stopPropagation(); onSave(client.id, formData); }} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white px-8 py-3 rounded-xl font-semibold transition-all shadow-lg">
